...
// Nombre d'inscriptions par page
...
function getValidatedInscriptions($limit, $offset, $search) {
    $pdo = getConnection();
    // Utilisation uniquement de paramètres nommés
    $sql = "SELECT * FROM inscriptions WHERE etat = 'validé' AND nom LIKE :search LIMIT :limit OFFSET :offset";
...
function getTotalValidatedInscriptions($search) {
    $pdo = getConnection();
    $sql = "SELECT COUNT(*) FROM inscriptions WHERE etat = 'validé' AND nom LIKE :search";
    $stmt = $pdo->prepare($sql);
    $stmt->bindValue(':search', '%' . $search . '%', PDO::PARAM_STR);
    $stmt->execute();
...
$inscriptions = getValidatedInscriptions($limit, $offset, $search);
